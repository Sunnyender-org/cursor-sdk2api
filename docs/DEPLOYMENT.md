...
- New sessions use model-aware round-robin across compatible accounts. Tool continuation, completed follow-up, and exact persisted restart recovery stay bound to the original credential fingerprint. Before semantic output, managed mode may try each remaining compatible account once, and admits no new account once one `FIRST_EVENT_TIMEOUT_MS` has elapsed since the first pre-semantic failure; an admitted attempt still gets its own first-event timeout, so the bound is a small multiple of one timeout rather than one per account. If the original account is removed, a self-contained tool transcript may cold-branch to another compatible account.
